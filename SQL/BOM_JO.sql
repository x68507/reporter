...
WITH jo_tree AS(
	SELECT m.fjobno,m.fsub_from,m.fpartno,m.fpartrev,convert(varchar(255),'') indent
		, 1 AS Level, CAST(m.identity_column AS VARCHAR(255)) AS Path
		FROM jomast m
		WHERE  m.fjobno like @JO
	UNION ALL
	SELECT m.fjobno,m.fsub_from,m.fpartno,m.fpartrev,convert(varchar(255),replicate('&nbsp',4*(Level))) indent
...
SELECT 
row_number() over (order by path) 'Row#*',Level,(mm.indent+j.fbompart) '!Part | Part',mm.fpartrev 'REV',i.fdescript 'Description | Description'
